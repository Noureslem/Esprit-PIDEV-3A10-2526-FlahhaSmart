(function () {
    ...
    function normalizeBackendTips(tips) {
        ...
        return tips.map(function (tip) {
            return {
                title: tip.title || 'Conseil',
                description: tip.description || '',
                color: tip.color || 'slate',
                icon: tip.icon || '',
    ...
    window.initWeatherDashboard = function initWeatherDashboard(scope) {
        ...
        var scoreValue = root.querySelector('#agriScoreValue');
        var scoreCategory = root.querySelector('#agriScoreCategory');
        ...
        async function loadWeather(city) {
            ...
            try {
                ...
                var backendTips = normalizeBackendTips(data.advice);
                renderTips(tipsList, backendTips.length ? backendTips : buildTips(data));

                if (scoreValue && scoreCategory) {
                    var score = data.agricultural_score || {};
                    var resolvedScore = toNumber(score.score, 0);
                    scoreValue.textContent = String(resolvedScore);
                    scoreCategory.textContent = score.category || 'conditions a risque';
                }
            } catch (error) {

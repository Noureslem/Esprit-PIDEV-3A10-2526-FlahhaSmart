...
final class EquipementController extends AbstractController
{
    ...
    public function index(Request $request, EquipementRepository $equipementRepository): Response
    {
        ...
        $equipements = $equipementRepository->search($nom, $type, $etat, $sort, $direction, $limit, $offset);
    ...
    public function delete(Request $request, Equipement $equipement, EntityManagerInterface $entityManager): Response
    {
        ...
        if (strtolower($equipement->getEtat()) === 'réservé') {
            $this->addFlash('error', $this->trans('flash.equipement.reserved_cannot_delete'));
            return $this->redirectToRoute('app_equipement_index', [], Response::HTTP_SEE_OTHER);
        }
        ...
        if ($this->isCsrfTokenValid('delete'.$equipement->getId(), $request->getPayload()->getString('_token'))) {
            ...
            $entityManager->flush();
            $this->addFlash('success', $this->trans('flash.equipement.deleted_success'));
        }
    ...
    public function bulkDelete(Request $request, BulkDeleteService $bulkDeleteService): Response
    {
        ...
        if (!$this->isCsrfTokenValid('bulk_delete_equipement', $token)) {
            $this->addFlash('error', $this->trans('flash.equipement.csrf_invalid'));
        ...
        if ($result->deletedCount() > 0) {
            $this->addFlash('success', $this->trans('flash.equipement.deleted_count', ['%count%' => $result->deletedCount()]));
        }

        if ($result->notFoundCount() > 0) {
            $this->addFlash('warning', $this->trans('flash.equipement.not_found_count', ['%count%' => $result->notFoundCount()]));
        }

        if ($result->failedCount() > 0) {
            $this->addFlash('error', $this->trans('flash.equipement.failed_count', ['%count%' => $result->failedCount()]));
        }

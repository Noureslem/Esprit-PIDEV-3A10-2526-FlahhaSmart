  ...
  function initChatbot(root) {
    ...
    async function sendMessage(event) {
      ...
      try {
        ...
        console.log('Chatbot API response:', { status: response.status, data });

        if (!response.ok) {
          const errorMsg = data?.error || `HTTP ${response.status}: An error occurred. Please try again.`;
          ...
          const fallbackMsg = 'Unable to process response. Please try again.';
        ...
        messages.push({
          type: 'bot',
          content: 'Network error. Please check your connection and try again. (' + error.message + ')',
          timestamp: new Date().toISOString(),
          isError: true,
        });
      } finally {
